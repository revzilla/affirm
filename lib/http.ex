defmodule Affirm.HTTP do
  ...
  def process_response({:ok, %{status_code: code, body: body}})
    ...
    do: {:ok, body}
  def process_response({:ok, %{status_code: 400}}),
  ...
  def fetch_error_code(error_code) do
    ...
    code_map =
      %{
        ...
        "capture-greater-instrument" => "Charges on this instrument cannot be captured for more than the authorization hold amount.",
        "capture-unequal-instrument" => "Charges on this instrument cannot be captured for an amount unequal to authorization hold amount.",
